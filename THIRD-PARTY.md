THIRD-PARTY NOTICES

This action downloads or uses the following components at runtime. They are
NOT redistributed as part of this project but are fetched on demand during
execution:

  - mitmproxy
    License: MIT
    https://github.com/mitmproxy/mitmproxy/blob/main/LICENSE

  - OWASP ZAP (Zed Attack Proxy)
    License: Apache License 2.0
    https://github.com/zaproxy/zaproxy/blob/main/LICENSE

  - ZAP Docker Image (ghcr.io/zaproxy/zaproxy)
    License: Apache License 2.0
    https://github.com/zaproxy/zaproxy/blob/main/LICENSE

  - AWS CLI / boto3
    License: Apache License 2.0
    https://github.com/aws/aws-cli/blob/develop/LICENSE.txt

  - MinIO (test dependency only)
    License: GNU AGPLv3
    https://github.com/minio/minio/blob/master/LICENSE

  - GitHub CodeQL Action (github/codeql-action)
    License: MIT
    https://github.com/github/codeql-action/blob/main/LICENSE

  - caseware/codeql-arm64-compat
    License: MIT
    https://github.com/caseware/codeql-arm64-compat/blob/master/LICENSE

Users are responsible for ensuring their use of OWASP ZAP and other
scanning tools complies with their respective licenses and applicable laws.
